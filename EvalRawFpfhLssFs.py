...
root_dir = './data'
read_dir1 = './calculated_ADscore/raw_iss_fs/'
read_dir2 = './calculated_ADscore/fpfh_iss_fs/'
image_rocaucs_list = []
pixel_rocaucs_list = []
image_aupr_list = []
pixel_aupr_list = []
...
for real_class in real_3d_classes:
    ...
    for data, mask, label, path in test_loader:
        ...
        target_path_1 = os.path.join(read_dir1,real_class,path_list[-1]+'_point_level.npy')
        target_path_2 = os.path.join(read_dir2,real_class,path_list[-1]+'_point_level.npy')
        point_anomaly_score_1 = np.load(target_path_1)
        point_anomaly_score_2 = np.load(target_path_2)
        
        target_path_1 = os.path.join(read_dir1,real_class,path_list[-1]+'_obj_level.npy')
        target_path_2 = os.path.join(read_dir2,real_class,path_list[-1]+'_obj_level.npy')
        image_anomaly_score_1 = np.load(target_path_1)
        image_anomaly_score_2 = np.load(target_path_2)
        
        rate=0.01
        pixel_preds_tmp = (1-rate)*point_anomaly_score_1+rate*point_anomaly_score_2       
        rate=0.001
        image_preds_tmp = (1-rate)*(np.max(image_anomaly_score_1))+rate*np.max(image_anomaly_score_2)      
