...
class PC_Features(torch.nn.Module):
    ...
    def compute_s_s_map(self, patch, feature_map_dims, mask, label, origin_data, sample_idx, source_path, target_path):
        ...
        full_s_map = fill_missing_values(sample_data,s_map,origin_data, k=3)
        '''
    ...
    def compute_anomaly_map_part(self, patch, origin_data, l):
        if(len(self.patch_lib)==1):
            cur_patch_lib  = self.patch_lib[0]
        else:
            cur_patch_lib = torch.cat(self.patch_lib)
        ...
        #full_s_map = fill_missing_values(patch,s_map,origin_data, k=10)
        #vis_pointcloud_anomalymap(origin_data,full_s_map)
        if l<5: #######如果某一个part里的点数很少，那么我们认为这个part不包含任何anomaly，直接把part整体的anomaly调到最低
            s = torch.tensor(0)
        #return s, full_s_map
        return s, s_map
    
        
    def compute_anomaly_map_part_fpfh(self, patch, sampled_raw_part, reg_raw_part, l):
        if(len(self.patch_lib)==1):
            cur_patch_lib  = self.patch_lib[0]
        else:
            cur_patch_lib = torch.cat(self.patch_lib)
        ...
        #assert(sampled_raw_part.shape[0]>3)
        full_s_map = fill_missing_values(sampled_raw_part,s_map,reg_raw_part, 1)
        #vis_pointcloud_anomalymap(origin_data,full_s_map)
        if l<5: #######如果某一个part里的点数很少，那么我们认为这个part不包含任何anomaly，直接把part整体的anomaly调到最低
            s = torch.tensor(0)
        return s, full_s_map
