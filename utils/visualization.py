...
def vis_pointcloud_np_two_diff_color(xyz=None,xyz2=None):
    """
    xyz = numpy.array N*3
    """
    pcd = o3d.geometry.PointCloud()
...
def create_geometry_at_points(points):
    ...
    return geometries
def vis_pointcloud_parts(xyz=None, cluster_idx=None, anchor_points=None):
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(xyz)
    colors = plt.get_cmap("tab20")(cluster_idx / anchor_points.shape[0])#カラーマップはplt.get_cmap()で取得します。取得したカラーマップに数値（0.0 ~ 1.0）を渡すことで、そのカラーマップ内の色を使うことができます。
    pcd.colors = o3d.utility.Vector3dVector(colors[:, :3])
    pcd_clusters = o3d.geometry.PointCloud()
    pcd_clusters.points = o3d.utility.Vector3dVector(anchor_points)
    highlight_pnts = create_geometry_at_points(pcd_clusters.points)
    o3d.visualization.draw([pcd,highlight_pnts],
                                      #zoom=0.5,
                                      #front=[0,0,1],
                                      #lookat=[0,1,0],
                                      #up=[1,0,0],
